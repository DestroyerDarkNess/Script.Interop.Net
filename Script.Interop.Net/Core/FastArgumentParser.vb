...
Namespace Core
            ...
            While strEnum.MoveNext()
                ...
                Else
                    If LastArg IsNot Nothing Then
                        ...
                        If LastArg.Value Is Nothing Then LastArg.Value = New List(Of String)
                        LastArg.Value.Add(strEnum.Current)
                        Continue While
                    End If
                End If
    ...
    Public Class IArgument
        ...
        Public Property Value As List(Of String) = Nothing
        ...
        Public Function ConvertType() As Object

            If Value IsNot Nothing Then
                Dim TypeName As String = Name.Substring(1, Name.Length - 1)

                '     Console.WriteLine("ConvertType " & TypeName)

                Select Case TypeName.ToLower

                    Case "bool" : Return Boolean.Parse(Value.FirstOrDefault.ToString())
                    Case "sbyte" : Return SByte.Parse(Value.FirstOrDefault.ToString())
                    Case "byte" : Return Byte.Parse(Value.FirstOrDefault.ToString())
                    Case "short" : Return Int16.Parse(Value.FirstOrDefault.ToString())
                    Case "ushort" : Return UInt16.Parse(Value.FirstOrDefault.ToString())
                    Case "int" : Return Int32.Parse(Value.FirstOrDefault.ToString())
                    ...
                    Case "double" : Return Double.Parse(Value.FirstOrDefault.ToString())
                    Case "decimal" : Return Decimal.Parse(Value.FirstOrDefault.ToString())
                    Case "char" : Return Char.Parse(Value.FirstOrDefault.ToString())
                    Case "string" : Return Value.FirstOrDefault.ToString()
                    Case Else
